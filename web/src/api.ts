...
export interface Spec {
  ...
  description: string;
  assumptions: string[];
  sources: { name: string; columns: Record<string, string> }[];
  steps: Step[];
  output: string;
...
export interface Finding {
  ...
  title: string;
  detail: string;
  evidence: { step: string; row_index: number; row: Row };
}
export interface RunDetail extends Run {
  version: Version;
  steps: { name: string; status: string; error: string | null }[];
  checks: Check[];
  artifacts: Artifact[];
  findings: Finding[];
  logs: {
  ...
  resolved_inputs: Record<string, SourceVersion>;
}
