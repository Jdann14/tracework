...
def test_real_worker_and_idempotency(workspace):
    ...
    duplicate = enqueue(
        workspace, RunRequest(version_id=ver["id"], inputs={"orders": source["id"]}, request_key="one")
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "SELECT 1; SELECT 2",
        "SELECT * INTO output FROM orders",
    ],
)
...
def test_actual_engine_controls():
    ...
    for sql in [
        ...
        "SET enable_external_access=true",
        "INSTALL httpfs",
        "COPY (SELECT 1) TO '/tmp/tracework-escape.csv'",
        "ATTACH '/tmp/no.db' AS escape",
    ]:
