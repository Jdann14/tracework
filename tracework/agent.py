...
class AgentTools:
    ...
    def call(self, name, args):
        ...
        store.execute("INSERT INTO tool_events(investigation_id,tool,arguments_json,response_json,created_at) VALUES(?,?,?,?,?)", (self.job["id"],name,store.safe_error(store.dumps(args)) if name == "credentials" else redact_json(args),redact_json(result),store.now()))
        return result
    ...
    def dispatch(self, name, args):
        ...
        if name == "list_datasets":
            return [{"name":s["name"],"versions":[{"id":v["id"],"rows":v["rows"],"hash":v["hash"]} for v in s["versions"]]} for s in catalog]
        if name in ("inspect_dataset","sample_rows"):
            source = next((s for s in catalog if s["name"] == args["name"]),None)
            if not source:
                raise ValueError("Dataset not found")
            selected = next((v for v in source["versions"] if v["id"] == args.get("version_id")),source["versions"][0])
            self.inspected.add(source["name"])
            if name == "sample_rows":
                return {"version_id":selected["id"],"rows":selected["profile"]["sample"][:min(20,max(1,args.get("limit",10)))]}
            return {"version_id":selected["id"],"schema":selected["schema"],"profile":{k:v for k,v in selected["profile"].items() if k != "sample"}}
        if name == "explore_sql":
            with tempfile.TemporaryDirectory(prefix="tracework-explore-") as directory:
                ...
                path.write_text(store.dumps({"workspace_id":self.workspace_id,"inputs":inputs,"sql":args["sql"]}))
                result = subprocess.run([sys.executable,"-m","tracework.explore",str(path)],env=child_env(),capture_output=True,text=True,timeout=10)
                if result.returncode:
                    raise ValueError(result.stderr.splitlines()[-1] if result.stderr else "Exploration process failed")
        ...
        if name == "inspect_execution_error":
            ...
            return {k:run[k] for k in ("id","status","error","steps","checks","logs","version","inputs")}
        if name == "request_clarification":
            message = str(args["question"])[:3000]
            store.execute("UPDATE investigations SET status='clarification',message=? WHERE id=?",(message,self.job["id"]))
            self.terminal = True
        ...
        if name in ("propose_pipeline","propose_revision"):
            ...
            if self.job.get("failed_run_id"):
                count = store.one("SELECT count(*) AS n FROM investigations WHERE failed_run_id=? AND version_id IS NOT NULL",(self.job["failed_run_id"],))["n"]
                if count >= MAX_REPAIRS:
                    raise ValueError("Two repair proposals already exist for this failure; edit a version manually")
            created = create_version(self.workspace_id,spec,parent)
            store.execute("UPDATE investigations SET status='proposed',version_id=?,message=? WHERE id=?",(created["id"],"Review the SQL, checks and assumptions. Approval applies only to this immutable version.",self.job["id"]))
            self.terminal = True
            return created
        if name == "record_finding":
            ...
            row = rows[idx]
            fid = store.uid()
            store.execute("INSERT INTO findings VALUES(?,?,?,?,?,?)",(fid,run["id"],artifact["id"],"Evidence row selected by agent",store.dumps(row),store.dumps({"row_index":idx,"row":row,"step":artifact["step"]})))
...
def tool_schema(name,description,properties,required=None):
    return {"type":"function","name":name,"description":description,"parameters":{"type":"object","properties":properties,"required":required if required is not None else list(properties),"additionalProperties":False},"strict":False}
...
def tools_schema():
    ...
    return [
        ...
        tool_schema("explore_sql","Run bounded exploratory SELECT against source names; explicitly LIMIT to 100 rows",{"sql":string}),
        tool_schema("inspect_execution_error","Read a failed attempt and its checks",{"run_id":string}),
        tool_schema("propose_pipeline","Propose a validated pipeline for user approval",{"spec":PipelineSpec.model_json_schema()}),
        tool_schema("propose_revision","Propose a new immutable version of the supplied base",{"spec":PipelineSpec.model_json_schema()}),
        tool_schema("request_clarification","Ask for absent data or a business definition",{"question":string}),
        tool_schema("record_finding","Ground a finding in a successful output row",{"run_id":string,"artifact_id":string,"row_index":{"type":"integer"}}),
    ]
...
class OpenAIAdapter:
    def run(self, tools):
        ...
        if tools.job.get("base_version_id"):
            history.append({"role":"user","content":"Base version: " + store.dumps(version(tools.workspace_id,tools.job["base_version_id"])) + "; failed run: " + str(tools.job.get("failed_run_id"))})
        with httpx.Client(timeout=35) as client:
            for _ in range(12):
                response = client.post("https://api.openai.com/v1/responses",headers={"Authorization":f"Bearer {key}"},json={"model":model,"instructions":SYSTEM,"tools":tools_schema(),"input":history,"store":False,"max_output_tokens":12000})
                if response.status_code >= 400:
                    raise ValueError(f"OpenAI request failed (HTTP {response.status_code}). Check model access, credentials and quota; provider response omitted to protect secrets.")
                output = response.json().get("output",[])
                history.extend(output)
                calls = [item for item in output if item.get("type") == "function_call"]
                if not calls:
                    text = "\n".join(c.get("text","") for item in output for c in item.get("content",[]) if c.get("type") == "output_text")
                    tools.call("request_clarification",{"question":text or "Please clarify the desired analysis and business definitions."})
                    return
                for call in calls:
                    try:
                    ...
                    history.append({"type":"function_call_output","call_id":call["call_id"],"output":store.dumps(result)})
                    if tools.terminal:
                        return
        raise ValueError("Agent reached its 12-turn budget; simplify the question or inspect the recorded tools")
...
class DemoAdapter:
    def run(self, tools):
        ...
        if not required <= available:
            tools.call("request_clarification",{"question":"The deterministic commerce demo needs these sources: " + ", ".join(sorted(required-available)) + ". Load demo batch one, or enable the real provider for other data."})
            return
        ...
        if not recovery and not any(s in question for s in ("profit","contribution","marketing channel")):
            tools.call("request_clarification",{"question":"Demo mode implements the commerce contribution-profit scenario only. Ask: " + QUESTION + " For other questions, enable the real provider."})
            return
        for name in sorted(required):
            tools.call("inspect_dataset",{"name":name})
        tools.call("sample_rows",{"name":"orders","limit":5})
        tools.call("explore_sql",{"sql":"SELECT currency,count(*) AS rows FROM orders GROUP BY currency LIMIT 100"})
        if recovery:
            ...
            if not any(s in question for s in ("repair","recover","duplicate","spend_cents","fix")):
                tools.call("request_clarification",{"question":"The deterministic recovery accepts spend_cents and removes exact duplicate orders. Request this repair explicitly, or edit the recipe / enable the real provider for other changes."})
                return
        tools.call("propose_revision" if recovery else "propose_pipeline",{"spec":commerce_spec(tools.workspace_id,recovery).model_dump()})
...
def investigate(job_id):
    ...
    try:
        ...
        store.execute("UPDATE investigations SET status='failed',error=? WHERE id=?",(store.safe_error(exc),job_id))
