...
def version(workspace_id, version_id):
    result = store.one(
        "SELECT v.*,p.workspace_id FROM pipeline_versions v JOIN pipelines p ON p.id=v.pipeline_id WHERE v.id=? AND p.workspace_id=?",
...
def create_version(workspace_id, spec: PipelineSpec, parent_id=None):
    ...
    with store.transaction() as db:
        ...
        number = db.execute(
            "SELECT coalesce(max(number),0)+1 FROM pipeline_versions WHERE pipeline_id=?", (pid,)
        ).fetchone()[0]
        vid = store.uid()
        raw = spec.model_dump()
        db.execute(
            "INSERT INTO pipeline_versions VALUES(?,?,?,?,?,?,?)",
            (vid, pid, number, parent_id, store.dumps(raw), store.digest(raw), store.now()),
        )
...
def resolve_inputs(workspace_id, inputs):
    ...
    for name, vid in inputs.items():
        row = store.one(
            "SELECT v.*,s.name,d.path,d.schema_json,d.rows FROM source_versions v JOIN sources s ON s.id=v.source_id JOIN datasets d ON d.id=v.dataset_id WHERE v.id=? AND s.workspace_id=?",
            (vid, workspace_id),
        )
        if not row:
...
def enqueue(workspace_id, request: RunRequest):
    ...
    with store.transaction() as db:
        existing = db.execute(
            "SELECT * FROM runs WHERE workspace_id=? AND request_key=?", (workspace_id, request.request_key)
        ).fetchone()
        if existing:
        ...
        rid = store.uid()
        db.execute(
            "INSERT INTO runs(id,workspace_id,version_id,request_key,request_hash,inputs_json,parameters_json,settings_json,status,created_at) VALUES(?,?,?,?,?,?,?,?,?,?)",
            (
                rid,
                workspace_id,
                request.version_id,
                request.request_key,
                sha,
                store.dumps(request.inputs),
                store.dumps(params),
                store.dumps(request.settings.model_dump()),
                "queued",
                store.now(),
            ),
        )
        for step in spec.steps:
            db.execute(
                "INSERT INTO step_executions(run_id,name,status) VALUES(?,?,?)", (rid, step.name, "queued")
            )
...
def get_run(workspace_id, run_id):
    ...
    run["artifacts"] = store.all_rows(
        "SELECT a.*,d.rows,d.schema_json,d.profile_json FROM artifacts a JOIN datasets d ON a.dataset_id=d.id WHERE run_id=?",
        (run_id,),
    )
