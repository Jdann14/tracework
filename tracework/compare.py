...
def compare_runs(workspace_id, left_id, right_id, key="channel", metric="contribution_profit"):
    ...
    for name in sorted(set(left["inputs"]) | set(right["inputs"])):
        ...
        if (before or {}).get("id") != (after or {}).get("id"):
            changes.append({"source":name,"before":before,"after":after,
    ...
    logic = [{"step":n,"before":old_steps.get(n),"after":new_steps.get(n)} for n in sorted(set(old_steps)|set(new_steps)) if old_steps.get(n)!=new_steps.get(n)]
    result = {"left":left_id,"right":right_id,"input_changes":changes,"logic_changed":left["version"]["hash"] != right["version"]["hash"],"step_changes":logic,
              "parameters_changed":left["parameters"] != right["parameters"],"settings_changed":left["settings"] != right["settings"],
              "assumptions_before":left["version"]["spec"]["assumptions"],"assumptions_after":right["version"]["spec"]["assumptions"],"rows":[],"message":None,"key":key,"metric":metric}
    if left["status"] != "successful" or right["status"] != "successful":
        result["message"] = "Both runs must succeed for numeric comparison. Input and recipe changes remain inspectable."
        return result
    def values(run):
    ...
    for name in sorted(set(a)|set(b),key=str):
        ...
        try:
            before,after = Decimal(str(x[metric])) if x else Decimal(0), Decimal(str(y[metric])) if y else Decimal(0)
        except InvalidOperation as exc:
            raise ValueError("Comparison metric must contain numeric values") from exc
        result["rows"].append({"key":name,"before":str(before),"after":str(after),"delta":str(after-before),"before_row":x,"after_row":y})
    return result
