...
def finish_run(run_id, status, message):
    with store.transaction() as db:
        db.execute("UPDATE runs SET status=?,error=?,finished_at=? WHERE id=? AND status IN ('running','queued')", (status, message, store.now(), run_id))
        db.execute("UPDATE step_executions SET status=?,error=?,finished_at=? WHERE run_id=? AND status='running'", (status, message, store.now(), run_id))
        db.execute("UPDATE step_executions SET status=?,finished_at=? WHERE run_id=? AND status='queued'", ("cancelled" if status == "cancelled" else "skipped", store.now(), run_id))
...
def recover_stale(seconds=15):
    ...
    for run in store.all_rows("SELECT id FROM runs WHERE status='running' AND heartbeat<?", (cutoff,)):
        finish_run(run["id"], "failed", "Worker heartbeat expired. Interrupted attempt preserved; queue a new run to recover.")
    store.execute("UPDATE investigations SET status='failed',error='Worker heartbeat expired; submit a new investigation' WHERE status='running' AND heartbeat<?", (cutoff,))
...
def claim(table):
    ...
    with store.transaction() as db:
        row = db.execute(f"SELECT * FROM {table} WHERE status='queued' ORDER BY created_at LIMIT 1").fetchone()
        if not row:
            return None
...
def tick():
    store.execute("INSERT INTO worker_state VALUES(1,?) ON CONFLICT(id) DO UPDATE SET heartbeat=excluded.heartbeat", (store.now(),))
...
def supervise(job, table="runs"):
    ...
    process = subprocess.Popen([sys.executable, "-m", module, job["id"]], env=env,
    ...
    try:
        ...
        if reason or row["status"] == "running":
            ...
            if is_run:
                ...
                store.execute("UPDATE investigations SET status='failed',error=? WHERE id=?", (reason, job["id"]))
