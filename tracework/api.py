...
def health():
    ...
    age = (
        (datetime.now(timezone.utc) - datetime.fromisoformat(worker["heartbeat"])).total_seconds()
        if worker
        else None
    )
    return {
        "status": "ok",
        "worker_online": age is not None and age < 10,
        "worker_heartbeat": worker["heartbeat"] if worker else None,
        "provider": os.getenv("TRACEWORK_PROVIDER", "demo"),
...
def workspace_detail(wid: str):
    ...
    pipelines = store.all_rows(
        "SELECT * FROM pipelines WHERE workspace_id=? ORDER BY created_at DESC", (wid,)
    )
    for pipeline in pipelines:
        pipeline["versions"] = store.all_rows(
            "SELECT v.*,a.approved_at FROM pipeline_versions v LEFT JOIN approvals a ON a.version_id=v.id WHERE pipeline_id=? ORDER BY number DESC",
            (pipeline["id"],),
        )
    investigations = store.all_rows(
        "SELECT * FROM investigations WHERE workspace_id=? ORDER BY created_at", (wid,)
    )
    for investigation in investigations:
        investigation["events"] = store.all_rows(
            "SELECT * FROM tool_events WHERE investigation_id=? ORDER BY id", (investigation["id"],)
        )
    return {
        "sources": sources(wid),
        "pipelines": pipelines,
        "runs": store.all_rows("SELECT * FROM runs WHERE workspace_id=? ORDER BY created_at DESC", (wid,)),
...
def download(wid: str, did: str, format: Literal["parquet", "csv"] = "parquet"):
    ...
    return Response(
        ...
        media_type="text/csv",
        headers={"Content-Disposition": f'attachment; filename="{record["name"]}.csv"'},
    )
...
def ask(wid: str, body: Question):
    ...
    iid = store.uid()
    store.execute(
        "INSERT INTO investigations(id,workspace_id,question,provider,status,base_version_id,failed_run_id,created_at) VALUES(?,?,?,?,?,?,?,?)",
        (
            iid,
            wid,
            body.question,
