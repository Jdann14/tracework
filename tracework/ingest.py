...
def profile_table(table):
    ...
    for field in table.schema:
        ...
        count, distinct = con.execute(f"SELECT count(*)-count({col}), count(DISTINCT {col}) FROM data").fetchone()
        ...
        columns.append({"name": field.name, "type": str(field.type), "nullable": field.nullable,
                        "missing": count, "distinct": distinct, **stats})
    con.close()
...
def normalize(source, target):
    ...
    if source.suffix == ".parquet":
        ...
        table = con.execute("SELECT * FROM read_csv(?, header=true, sample_size=-1) LIMIT ?",
                            [str(source), MAX_ROWS + 1]).to_arrow_table()
        con.close()
...
def ingest(workspace_id, name, filename, data):
    ...
    existing = store.one("SELECT v.* FROM source_versions v JOIN sources s ON s.id=v.source_id WHERE s.workspace_id=? AND s.name=? AND v.hash=?", (workspace_id, name, sha))
    if existing:
        return existing
    ...
    try:
        result = subprocess.run([sys.executable, "-m", "tracework.ingest", str(original), str(normalized)],
                                capture_output=True, text=True, timeout=35, env=child_env())
        if result.returncode:
            raise ValueError("Ingestion failed: " + store.safe_error(result.stderr.splitlines()[-1] if result.stderr else "worker exited"))
    ...
    with store.transaction() as db:
        source = db.execute("SELECT id FROM sources WHERE workspace_id=? AND name=?", (workspace_id, name)).fetchone()
        sid = source["id"] if source else store.uid()
        if not source:
            db.execute("INSERT INTO sources VALUES(?,?,?)", (sid, workspace_id, name))
        existing = db.execute("SELECT * FROM source_versions WHERE source_id=? AND hash=?", (sid, sha)).fetchone()
        if existing:
...
def child_env():
    import os
    return {k: v for k, v in os.environ.items() if k in ("PATH", "SYSTEMROOT", "TMPDIR", "LANG", "VIRTUAL_ENV")} | {"TRACEWORK_DATA_DIR": str(store.root())}
...
def sources(workspace_id):
    ...
    for source in records:
        source["versions"] = store.all_rows("SELECT v.*,d.rows,d.schema_json,d.profile_json FROM source_versions v JOIN datasets d ON d.id=v.dataset_id WHERE source_id=? ORDER BY created_at DESC", (source["id"],))
    return records
